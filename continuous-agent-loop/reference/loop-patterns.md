...
Migrated from the retired `autonomous-loops` skill (2026-06-12). Detailed recipes
for each loop pattern. The inline SKILL.md handles selection; this file holds the
implementation detail.

For the two most sophisticated patterns, defer to their dedicated skills:
- RFC-driven DAG orchestration → `ralphinho-rfc-pipeline`
- Persistent session REPL → `nanoclaw-repl`
...
| Ralphinho / RFC-Driven DAG (`ralphinho-rfc-pipeline`) | High | Large features, multi-unit parallel work with merge queue |
...
> If you can't figure out a loop like this, it means you can't even drive the LLM
> to fix your code in interactive mode.

The `claude -p` flag runs Claude Code non-interactively with a prompt, exits when
done. Chain calls to build a pipeline:
...
3. **Negative instructions are dangerous** — Don't say "don't test type systems." Instead, add a separate cleanup step (see De-Sloppify Pass).
...
**With model routing:**
```bash
# Research with the deepest-reasoning model
claude -p --model opus "Analyze the codebase architecture and write a plan for adding caching..."

# Implement with the fast, capable coding model
claude -p "Implement the caching layer according to the plan in docs/caching-plan.md..."

# Review with the deepest-reasoning model (thorough)
...
**A two-prompt system** that orchestrates parallel sub-agents for
specification-driven generation. Developed by disler (credit: @disler).
...
Don't rely on agents to self-differentiate. The orchestrator **assigns** each agent
a specific creative direction and iteration number. This prevents duplicate concepts
...
creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary
(credit: @AnandChowdhary).
...
> **Warning:** Install continuous-claude from its repository after reviewing the
> code. Do not pipe external scripts directly to bash.
...
Claude reads this file at iteration start and updates it at iteration end. This
bridges the context gap between independent `claude -p` invocations.
...
Three consecutive iterations signaling completion stops the loop, preventing wasted
runs on finished work.
...
## 4. The De-Sloppify Pass

**An add-on pattern for any loop.** Add a dedicated cleanup/refactor step after
each Implementer step.
...
Adding "don't test type systems" or "don't add unnecessary checks" to the Implementer
prompt has downstream effects:
...
Instead of constraining the Implementer, let it be thorough. Then add a focused
cleanup agent:
...
> add a separate de-sloppify pass. Two focused agents outperform one constrained agent.
...
4. **Tiered model routing in simpler loops** — Even in a sequential pipeline, route simple tasks to the fast model and complex tasks to the deepest-reasoning model:
   ```bash
...
| Project | Author | Link |
|---------|--------|------|
| Ralphinho | enitrat | credit: @enitrat (see `ralphinho-rfc-pipeline` skill) |
| Infinite Agentic Loop | disler | credit: @disler |
| Continuous Claude | AnandChowdhary | credit: @AnandChowdhary |
| NanoClaw | ECC | `nanoclaw-repl` skill / `/claw` command in this repo |
